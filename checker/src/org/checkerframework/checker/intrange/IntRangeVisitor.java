...
import org.checkerframework.checker.intrange.qual.EmptyRange;
import org.checkerframework.checker.intrange.qual.FullIntRange;
...
public class IntRangeVisitor extends BaseTypeVisitor<IntRangeAnnotatedTypeFactory> {
        ...
        if (args.size() == 2
            ...
            if (isCoveredKind(expFrom.getKind()) && isCoveredKind(expTo.getKind())) {
                ...
                if (valueFrom > valueTo) {
                    checker.report(
                            Result.warning("from.greater.than.to", valueFrom, valueTo), node);
                }
            }
        }
    ...
    public Void visitBinary(BinaryTree node, Void p) {
        ...
        if (!isCoveredTypeKind(typeLeft.getKind()) || !isCoveredTypeKind(typeRight.getKind())) {
            ...
            case UNSIGNED_RIGHT_SHIFT:
                {
                    ...
                    if (rangeRight.from < 0 || rangeRight.to > 31) {
                        ...
                        checker.report(
                                Result.warning(
                                        "shift.out.of.range", rangeRight.from, rangeRight.to),
                                node.getRightOperand());
                    }
                    break;
                }
    ...
    public Void visitNewArray(NewArrayTree node, Void p) {
        ...
        for (ExpressionTree dim : dimensions) {
            ...
            if (rangeDim.from < 0) {
                checker.report(
                        Result.warning(
                                "possible.negative.array.dimension", rangeDim.from, rangeDim.to),
                        dim);
            }
        }
    ...
    public Void visitArrayAccess(ArrayAccessTree node, Void p) {
        ...
        if (idxRange.from < 0) {
            checker.report(
                    Result.warning("possible.negative.array.index", idxRange.from, idxRange.to),
                    idxNode);
        }
