...
public abstract class AbstractQualifierPolymorphism implements QualifierPolymorphism {
    ...
    public void resolve(NewClassTree tree, AnnotatedExecutableType type) {
        ...
        List<AnnotatedTypeMirror> arguments;
        AnnotationMirrorMap<AnnotationMirror> instantiationMapping;
        if (tree.getArguments().size() + 1 == parameters.size()) {
            List<AnnotatedTypeMirror> actualParams = new ArrayList<>();
            // happens for anonymous constructors of inner classes
            for (int i = 1; i < parameters.size(); i++) {
                actualParams.add(parameters.get(i));
            }
            arguments =
                    AnnotatedTypes.getAnnotatedTypes(
                            atypeFactory, actualParams, tree.getArguments());
            instantiationMapping = collector.visit(arguments, actualParams);
        } else {
            arguments =
                    AnnotatedTypes.getAnnotatedTypes(atypeFactory, parameters, tree.getArguments());
            instantiationMapping = collector.visit(arguments, parameters);
        }
